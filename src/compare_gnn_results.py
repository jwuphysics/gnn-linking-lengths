...
def rf_cv(df, feature_cols, target_col, kfold_col, nfold=3, **rf_kwargs):
    """Cross valiate using random forest model"""

    rf_preds = []
    for k in range(nfold):
        valid_mask = (df[kfold_col] == k)
        
        X_train = df[feature_cols][~valid_mask].values
        y_train = df[target_col][~valid_mask].values.flatten()
        X_valid = df[feature_cols][valid_mask].values
        y_valid = df[target_col][valid_mask].values.flatten()
        ...
        rf_preds.append(rf.predict(X_valid))

    rf_preds = np.concatenate(rf_preds)
    return rf_preds


def collate_comparison_results(D_link, dmo_only=False, n_epochs=300, aggr="multi"):
    ...
    kfold = []
    for k in range(3):
        dmo += [pd.read_csv(f"{results_path}/gnn-{n_epochs}-{aggr}-dmo-fold_{k+1}.cat", dtype={"is_central": bool})]
        if not dmo_only:
            hydro += [pd.read_csv(f"{results_path}/gnn-{n_epochs}-{aggr}-hydro-fold_{k+1}.cat")]
    ...
    if not dmo_only:
        ...
        df = dmo.reset_index().set_index("subhalo_id")
    ...
    #     xgb_Mstar_disperse_hydro = xgboost_cv(df, halo_hydro_features + disperse_features, target, "k_fold", nfold=3, verbosity=0).astype(float)
     ...
     # run rf models
    # rf_Mstar_dmo = rf_cv(df, halo_dmo_features, target, "k_fold", nfold=3).astype(float)
    # rf_Mstar_overdensity_dmo = rf_cv(df, halo_dmo_features + overdensity_features, target, "k_fold", nfold=3).astype(float)
    # rf_Mstar_disperse_dmo = rf_cv(df, halo_dmo_features + disperse_features, target, "k_fold", nfold=3).astype(float)
    # if not dmo_only:
    #     rf_Mstar_hydro = rf_cv(df, halo_hydro_features, target, "k_fold", nfold=3).astype(float)
    #     rf_Mstar_overdensity_hydro = rf_cv(df, halo_hydro_features + overdensity_features, target, "k_fold", nfold=3).astype(float)
    #     rf_Mstar_disperse_hydro = rf_cv(df, halo_hydro_features + disperse_features, target, "k_fold", nfold=3).astype(float)
...
if __name__ == "__main__":
    for D_link in [0.3, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 5, 7.5, 10]:
        ...
        if not os.path.isfile(cv_results_fname) or overwrite:
            if verbose: print(f"Collating results for {D_link} Mpc")
            cv_results = collate_comparison_results(D_link, dmo_only=True)
